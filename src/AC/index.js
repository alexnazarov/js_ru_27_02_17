import {INCREMENT, DELETE_ARTICLE, CHANGE_DATE_RANGE, CHANGE_SELECTION, ADD_COMMENT,
    LOAD_ALL_ARTICLES, LOAD_ARTICLE_BY_ID, LOAD_COMMENTS_BY_ID, START, SUCCESS, FAIL} from '../constants'
import $ from 'jquery'
...
export function loadCommentsById(id) {
    return (dispatch) => {
        dispatch({
            type: LOAD_COMMENTS_BY_ID + START,
            payload: { id }
        })
        ...
        setTimeout(() => {
            $.get(`/api/comment?article=${id}`)
                .done(response => dispatch({
                    type: LOAD_COMMENTS_BY_ID + SUCCESS,
                    payload: { response, id }
                }))
                .fail(error => dispatch({
                    type: LOAD_COMMENTS_BY_ID + FAIL,
                    payload: { error, id }
                }))
        }, 1000)
    }
